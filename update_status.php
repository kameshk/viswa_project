<?php
	...
	$files = scandir($data_directory_status_path);
	foreach($files as $filename){
		$file_explode	=	explode('.',$filename);
		$file_extn		=	end($file_explode);
		if($file_extn == 'csv'){
		//	echo $filename."</br>";
			$inputFileName		=	$data_directory_status_path."/".$filename;
		...
		//	echo $inputFileName."</br>";
			...
			for ($row = 2; $row <= $highestRow; $row++){ 
				...
				*/
				...
				$vehicle_mode		=	$sheet->getCell($col++.$row)->getValue();
				$odometer			=	$sheet->getCell($col++.$row)->getValue();
				$speed				=	$sheet->getCell($col++.$row)->getValue();
				$soc				=	$sheet->getCell($col++.$row)->getValue();
				$dte				=	$sheet->getCell($col++.$row)->getValue();
				$ac_status			=	$sheet->getCell($col++.$row)->getValue();
				...
				$getLastEntryQuery		=	"SELECT data_timestamp, zero_gap_frequency FROM data_table_1 WHERE vin='$vin' && response_timestamp<='$response_timestamp' order by response_timestamp desc limit 0,1";
			...
			//	echo $vin."||".$vehicle_mode."||".$odometer."||".$speed."||".$soc."||".$dte."||".$ac_status."||".$data_timestamp."||".$response_timestamp."||".$time_gap."||".$zero_gap_frequency."</br>";
				...
				$InsertQuery	=	"INSERT INTO data_table_1(key_column, vin, vehicle_mode, odometer, speed, soc, dte, ac_status, data_timestamp, response_timestamp, time_gap, zero_gap_frequency, filename) VALUES('$key_column', '$vin', '$vehicle_mode', '$odometer', '$speed', '$soc', '$dte', '$ac_status', '$data_timestamp', '$response_timestamp', '$time_gap', '$zero_gap_frequency', '$inputFileName') ON DUPLICATE KEY UPDATE vin='$vin', vehicle_mode='$vehicle_mode', odometer='$odometer', speed='$speed', soc='$soc', dte='$dte', ac_status='$ac_status', data_timestamp='$data_timestamp', time_gap='$time_gap', zero_gap_frequency='$zero_gap_frequency', filename='$inputFileName'";
			...
			shell_exec("mv '$inputFileName' '/processed/status/$filename'");
