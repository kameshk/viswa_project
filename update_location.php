<?php
	...
	$files = scandir($data_directory_location_path);
	foreach($files as $filename){
		$file_explode	=	explode('.',$filename);
		$file_extn		=	end($file_explode);
		if($file_extn == 'csv'){
		//	echo $filename."</br>";
			$inputFileName		=	$data_directory_location_path."/".$filename;
		...
		//	echo $inputFileName."</br>";
			...
			for ($row = 2; $row <= $highestRow; $row++){ 
				...
				*/
				...
				$latitude			=	$sheet->getCell($col++.$row)->getValue();
				$longitude			=	$sheet->getCell($col++.$row)->getValue();
				$latitude_direction	=	$sheet->getCell($col++.$row)->getValue();
				$longitude_direction=	$sheet->getCell($col++.$row)->getValue();
				$gps_validity_flag	=	$sheet->getCell($col++.$row)->getValue();
				$gps_speed			=	$sheet->getCell($col++.$row)->getValue();
				...
				$getLastEntryQuery		=	"SELECT data_timestamp, zero_gap_frequency FROM data_table_2 WHERE vin='$vin' && response_timestamp<='$response_timestamp' order by response_timestamp desc limit 0,1";
			...
			//	echo $vin."||".$vehicle_mode."||".$odometer."||".$speed."||".$soc."||".$dte."||".$ac_status."||".$data_timestamp."||".$response_timestamp."||".$time_gap."||".$zero_gap_frequency."</br>";
				...
				$InsertQuery	=	"INSERT INTO data_table_2(key_column, vin, latitude, longitude, latitude_direction, longitude_direction, gps_validity_flag, gps_speed, data_timestamp, response_timestamp, time_gap, zero_gap_frequency, filename) VALUES('$key_column', '$vin', '$latitude', '$longitude', '$latitude_direction', '$longitude_direction', '$gps_validity_flag', '$gps_speed', '$data_timestamp', '$response_timestamp', '$time_gap', '$zero_gap_frequency', '$inputFileName') ON DUPLICATE KEY UPDATE vin='$vin', latitude='$latitude', longitude='$longitude', latitude_direction='$latitude_direction', longitude_direction='$longitude_direction', gps_validity_flag='$gps_validity_flag ', gps_speed='$gps_speed', data_timestamp='$data_timestamp', time_gap='$time_gap', zero_gap_frequency='$zero_gap_frequency', filename='$inputFileName'";
			...
			shell_exec("mv '$inputFileName' '/processed/location/$filename'");
